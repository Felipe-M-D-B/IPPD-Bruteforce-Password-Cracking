	...
	if (idx < (len - 1)) {
		...
		for (c = 0; c < strlen(letters) && *ok==0; ++c) {
			...
			if(strncmp((char*)hash1, (char*)hash2, MD5_DIGEST_LENGTH) == 0){
				//varias threads podem ler e escrever em ok, protege o print e a escrita de ok
				#pragma omp critical
				{
					printf("found: %s\n", str);
					print_digest(hash2);
...
int main(int argc, char **argv) {
	...
	#pragma omp parallel for schedule(dynamic) shared(hash1, ok)
	for(len = 1; len <= lenMax; len++){
		//cada thread tem uma copia de str e hash2, isso evita condicoes de corrida
		char str[MAX+1];
		byte hash2[MD5_DIGEST_LENGTH]; // string hashes
